...
module.exports = function(config, updateTimestamp) {
  ...
  function endStream(cb) {
    ...
    const stream = this;
    compileIndex(mapToCategories(examples, null), stream);
    compileExamples(examples, stream);

    cb();
  }

  function compileIndex(categories, stream) {
    const args = {
  ...
  function compileExamples(examples, stream) {
  ...
  function mapToCategories(examples, currentExample) {
    const categories = [];
    let currentCategory;
    sort(examples).filter(exampleFile => !exampleFile.document.metadata.draft)
    .forEach(function(exampleFile) {
      if (!exampleFile.document.metadata.draft) {
        ...
        let selected = false;
        if (currentExample) {
          selected = exampleFile.title() == currentExample.title();
        }
